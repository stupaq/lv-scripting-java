...
public class example {
  ...
  private static void run(String[] args) throws Exception {
    if (args.length == 1) {
      ...
      vi.create();
      UID b1 = vi.inlineCNodeCreate("block 1", "block label 1");
      UID b2 = vi.inlineCNodeCreate("block 2", "block label 2");
      UID t1 = vi.inlineCNodeAddIO(b1, false, "out 1");
      UID t2 = vi.inlineCNodeAddIO(b2, true, "in 2");
      UID w1 = vi.connectWire(t1, t2, "wire 1");
      UID b3 = vi.inlineCNodeCreate("this block will be removed", "");
      vi.cleanUpDiagram();
